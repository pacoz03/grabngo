...
// <-- AGGIUNTO: Import per autenticazione e componente Punti
...
const homeStyles = StyleSheet.create({
    ...
    // <-- AGGIUNTO: Stile per la sezione dei punti
    pointsSection: {
...
export default function HomeScreen() {
    ...
    // <-- AGGIUNTO: Otteniamo session, profile e lo stato di caricamento direttamente dal context
    const { session, profile, loading: authLoading } = useAuth();

    useEffect(() => {
        (async () => { await Location.requestForegroundPermissionsAsync(); })();

        // La logica per caricare mappa, prodotti e offerte resta invariata
        // e non dipende più dalla sessione utente in questo componente.
